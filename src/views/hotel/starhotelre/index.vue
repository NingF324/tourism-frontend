...
<script setup name="Starhotelreviews">
import { listStarhotelreviews, getStarhotelreviews, delStarhotelreviews, addStarhotelreviews, updateStarhotelreviews } from "@/api/hotel/starhotelreviews";

const { proxy } = getCurrentInstance();

const starhotelreviewsList = ref([]);
...
function getList() {
  ...
  listStarhotelreviews(queryParams.value).then(response => {
    starhotelreviewsList.value = response.rows;
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["starhotelreviewsRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateStarhotelreviews(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addStarhotelreviews(form.value).then(response => {
...
function handleExport() {
  proxy.download('hotel/starhotelreviews/export', {
    ...queryParams.value
  }, `starhotelreviews_${new Date().getTime()}.xlsx`)
}
