    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['hotel:nonstarroomtypes:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['hotel:nonstarroomtypes:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['hotel:nonstarroomtypes:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['hotel:nonstarroomtypes:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="nonstarroomtypesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['hotel:nonstarroomtypes:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["nonstarroomtypesRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateNonstarroomtypes(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addNonstarroomtypes(form.value).then(response => {
...
function handleExport() {
  proxy.download('hotel/nonstarroomtypes/export', {
    ...queryParams.value
  }, `nonstarroomtypes_${new Date().getTime()}.xlsx`)
}
